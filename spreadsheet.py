...
def build_workbook(records: list[dict]) -> openpyxl.Workbook:
    ...
    for row_idx, record in enumerate(sorted_records, start=2):
        for col_idx, key in enumerate(keys, start=1):
            ...
            if key in _CURRENCY_KEYS and value is not None:
    ...
    for col_idx, (key, display_name) in enumerate(FIELDS, start=1):
        ...
        for row_idx in range(2, ws.max_row + 1):
            ...
            if key in _CURRENCY_KEYS:
                # Match Excel's rendered display for width estimation
